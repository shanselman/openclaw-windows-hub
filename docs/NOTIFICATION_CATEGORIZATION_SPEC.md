...
- Should per-agent rules be derived from the gateway session `channel` field (the same value mapped into the notification `Channel` metadata) when the Agent field is missing?
